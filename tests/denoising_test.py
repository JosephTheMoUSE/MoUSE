...
import numpy as np
from mouse.denoising.denoising import _diagonal_gaussian_filter, \
  _gaussian_filter
...
def test_symmetric_gaussian_filter():
    ...
    image = np.array([[1., 2., 30., 4.], [5., 66., 7., 8.],
                      [90., 10., 91., 12.]])
    ...
    middle_pixel_1 = np.sum(
        image[:, :3] * gaussian_kernel) / np.sum(gaussian_kernel)
    middle_pixel_2 = np.sum(
        image[:, 1:] * gaussian_kernel) / np.sum(gaussian_kernel)
