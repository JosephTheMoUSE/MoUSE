...
def eval_nn_detection(spec, model_name, thresholds=(-1, 0.1)):
    return {f'{model_name}({th})': find_USVs_nn(spec_data=spec,
                                                model_name=model_name,
                                                cache_dir=Path('.'),
                                                batch_size=1,
                                                confidence_threshold=th,
                                                tqdm_kwargs=dict(position=1, leave=False, desc=f'{model_name}({th})'))
...
def eval_gac_detection(spec, param_sets):
    ...
    for idx, params in enumerate(param_sets, 1):
        ...
        if filter_18:
            preds = [p for p in preds if p if (p.freq_end - p.freq_start) / 2 >= spec.freq_to_pixels(18000)]
        res[f'GAC-{idx}'] = preds
    return res


def denoise_spec(spec: SpectrogramData,
                 boxes=None,
                 file=None,
                 noise_len=1.0,
                 n_grad_freq=3,
    ...
    if spans:
        try:
            weights = np.array(
                [np.mean(np.quantile(spec.spec[:, s:e], 0.75, axis=1)) for s, e in spans if e - s > 1])
            span = spans[np.random.choice(len(spans), p=weights / np.sum(weights))]
        except:
            span = [0, spec.times.shape[0] - 1]
            noise_decrease = noise_decrease / 2
            warnings.warn(f"Error occurred while searching for noise. For file {file}\n"
                          "Span will be chosen randomly. Decreasing `noise_decrease` by half")
    else:
        span = [0, spec.times.shape[0] - 1]
        noise_decrease = noise_decrease / 2
        warnings.warn(f"Can't find sufficiently long span without calls. For file {file}\n"
                      "Span will be chosen randomly. Decreasing `noise_decrease` by half")
    ...
    noise_spec = clip_spectrogram(
        spec=spec, t_start=spec.times[span[0]], t_end=spec.times[span[1]]
    )
    noise_gate_filter(
        spectrogram=spec,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_thresholds', nargs='+', type=float, default=[-1, 0.1, 0.3, 0.5])
    ...
    annotations.rename(columns=lambda x: re.sub('_\(.*\)', '', x).replace(' ', '_').lower(), inplace=True)
    ...
    for file in tqdm(audio_files, position=0):
        ...
        spec = generate_spectrogram(waveform,
                                    sample_rate=sample_rate)

        usv_type = 'usv_type' if 'usv_type' in annotations.keys() else 'note'
        file_annotations = annotations.loc[annotations.begin_file == file, ['begin_time', 'end_time',
                                                                            'low_freq', 'high_freq', usv_type]]

        ground_truth = []
        for idx, (begin_time, end_time, low_freq, high_freq, cls) in file_annotations.iterrows():
            if cls == 'background':
                continue
            ...
            gt_box = SqueakBox(freq_start=freq_start_index, freq_end=freq_end_index,
                               t_start=time_start_index, t_end=time_end_index,
                               label=cls)
        ...
        param_sets = [
            {'preprocessing_fn': partial(segmentation.inverse_gaussian_gradient,
                                         sigma=7.0,
                                         alpha=200.0, ),
             'level_set': eroded_level_set_generator(threshold=0.75),
             'balloon': -1,
             'threshold': 0.8,
             'smoothing': 3,
             'iterations': 15,
            ...
            {'preprocessing_fn': partial(segmentation.inverse_gaussian_gradient,
                                         sigma=7.0,
                                         alpha=200.0, ),
             'denoise_fn': partial(denoise_spec,
                                   boxes=ground_truth,
                                   file=file,
                                   noise_len=1.0,
                                   n_grad_freq=3,
            ...
            {'preprocessing_fn': partial(segmentation.inverse_gaussian_gradient,
             ...
             'denoise_fn': partial(denoise_spec,
                                   ...
                                   n_std_thresh=1.0,
                                   noise_decrease=0.5),
             'level_set': eroded_level_set_generator(threshold=0.8),
             'balloon': -1,
             'threshold': 0.8,
             'smoothing': 2,
             'iterations': 15,
        ...
        for m, preds in chain(nn_model_preds.items(), preds_gac.items()):
            results[m]['iou'] += intersection_over_union_global(preds, ground_truth) * 100
